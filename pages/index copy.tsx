...
const getServerSideProps: GetServerSideProps = async ({ req, res }) => {
  ...
  const providers = await prisma.peer.findMany({
    where: { userId: user?.id, kind: 'provider' },
  })

  const consumers = await prisma.peer.findMany({
    where: { userId: user?.id, kind: 'consumer' },
  })
      ...
      <div className="main pt-12 text-xs">
        ...
        <div className="px-4 sm:px-8 md:px-12 pb-16">
          ...
          <ul role="list" className=" pb-12 grid grid-cols-2 gap-6 md:grid-cols-8 2xl:grid-cols-6">
            ...
            <li className="shadow-md col-span-2 border rounded-sm cursor-pointer hover:border-gray dark:hover:border-gray border-gray-lightest dark:border-gray-dark text-boring-black dark:text-boring-white bg-boring-white dark:bg-boring-black ">
              add consumer peer here
