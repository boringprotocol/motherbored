...
async function deletePeer(id: string): Promise<void> {
    ...
    if (result.ok) {
        ...
        toast.success(`Peer Deleted! Thanks for tidying up.`, {
            position: "top-center",
            autoClose: 5000,
        ...
        toast.error(`Oh no! We couldn't delete this peer. Try again.`, {
            position: "top-center",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
...
async function shovePeerConfig(id: string): Promise<void> {
    ...
    if (results.ok) {
        ...
        try {
            ...
            toast.error(
                <div className="">
                    <p className="text-lg">Shit dude!</p><br /><br />
                </div>,
                {
                    position: "top-center",
                    autoClose: false,
...
const ShowPeer: React.FC<Props> = (props) => {
    ...
    const [providerPeers, setProviderPeers] = useState([]);
    const [nameFilter, setNameFilter] = useState('');
    const uniqueCountries = Array.from(
        new Set(providerPeers.map((pp) => pp.country_code))
    );
    const [countryFilter, setCountryFilter] = useState("");


    const providerCountryCodes = providerPeers.map(provider => provider.country_code);
    ...
    const submitData = async (e: React.SyntheticEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                toast.success(
                    <div className="">
                        <h1 className="text-lg"><IoSaveOutline className="float-left mr-2" /> Peer Saved</h1>
    ...
    // we are doing provider_platform and consumer_platform to make it easier to add more platforms in the future
    let isCloudProvider = false
    if (props.peer.provider_kind == "cloud") {
        isCloudProvider = true
    }

    let islocalConsumer = false
    if (props.peer.consumer_platform == "pi") {
        islocalConsumer = true
    }
                ...
                <div className=" box col-start-1 col-span-4 sm:col-span-4 ">
                    ...
                    <form className="px-0 md:px-10" onSubmit={submitData}>
                        ...
                        <div className=" bg-boring-white dark:bg-boring-black text-boring-black dark:text-boring-white placeholder-boring-black dark:placeholder-boring-white border border-gray-lightest dark:border-gray-dark rounded-sm px-3 py-2 focus-within:border-blue focus-within:ring-1 focus-within:ring-blue">
                            <label htmlFor="name" className="block text-xs dark:text-gray">
                                <IoPricetagOutline className="float-left mr-2" /> Label
                            </label>
                            <input
                                type="text"
                                name="label"
                            ...
                            <>
                                <div className="pt-12">
                                    ...
                                    <select
                                        ...
                                        className="py-2 px-3 rounded-sm border border-gray bg-boring-black"
                                    >
                                    ...
                                    <input type="text" placeholder="Filter by name" className="py-2 px-3 rounded-sm border border-gray bg-boring-black" onChange={e => setNameFilter(e.target.value)} />
                                ...
                                <div className="pt-4">
                                    <Switch
                                        checked={enabled}
                                        onChange={setEnabled}
                                        className={`${enabled ? 'bg-black' : 'bg-gray-darker'}
          relative inline-flex h-[28px] w-[64px] shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75`}
            ...
            pointer-events-none inline-block h-[24px] w-[24px] transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out`}
                                            ...
                                            <>
                                                <div className="relative mt-1">
                                                    ...
                                                    <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-sm bg-boring-white dark:bg-boring-black text-boring-black dark:text-boring-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                                                        {props.providerPeers
                                                            .filter(pp => pp.country_code.includes(countryFilter) && pp.name.toLowerCase().includes(nameFilter.toLowerCase()))
                                                            .map((pp) => (
                                                                <Listbox.Option
                                                                    key={pp.id}
                                                                    value={pp}
                        ...
                        <div className="mt-6 py-6 border-t border-b border-gray-lightest dark:border-gray-dark">
                            ...
                            <a className="my-6 mr-2 inline-flex items-center rounded-xs border border-gray dark:border-gray-dark text-xs bg-white dark:bg-black px-3 py-2 text-boring-black dark:text-gray-light hover:bg-boring-white hover:opacity-80 active:opacity-60 shadow-md dark:shadow-sm dark:shadow-black active:shadow-sm" href="https://unconfigured.insecure.boring.surf/api/reboot"><IoRefreshOutline className="mr-2" /> Reboot </a><a className="my-6 mr-2 inline-flex items-center rounded-xs border border-gray dark:border-gray-dark text-xs bg-white dark:bg-black px-3 py-2 text-boring-black dark:text-gray-light hover:bg-boring-white hover:opacity-80 active:opacity-60 shadow-md dark:shadow-sm dark:shadow-black active:shadow-sm" href="https://unconfigured.insecure.boring.surf/" target="_blank" rel="noreferrer"><IoBugOutline className="mr-2" /> Debug </a><a className="my-6 mr-2 inline-flex items-center rounded-xs border border-gray dark:border-gray-dark text-xs bg-white dark:bg-black px-3 py-2 text-boring-black dark:text-gray-light hover:bg-boring-white hover:opacity-80 active:opacity-60 shadow-md dark:shadow-sm dark:shadow-black active:shadow-sm" href="https://unconfigured.insecure.boring.surf:19531/browse" target="_blank" rel="noreferrer"> <IoFileTrayFull className="mr-2" /> Logs</a>
                            <button className="my-6 inline-flex items-center rounded-xs border border-gray dark:border-gray-dark text-xs bg-white dark:bg-black px-3 py-2 text-boring-black dark:text-gray-light hover:bg-boring-white hover:opacity-80 active:opacity-60 shadow-md dark:shadow-sm dark:shadow-black active:shadow-sm" onClick={() => downloadPeerConfig(props.peer.id)}><IoDownloadOutline className="mr-2" /> boring.env</button>
                            <hr />
                            <form>
                                <button
                                    type="button"
